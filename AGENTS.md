...
- Add or update a single business with `python3 scripts/add_business.py`: edit the `RECORD` at the bottom (name, phone, address, website, `c` categories, `city`) and run it — it updates the master dataset, generates the detail page, updates each category page + its counts, bumps global totals, regenerates the search index, and adds to the sitemap. It upserts by slug (safe to re-run). Always run the audit afterward.
